...
2. Ir a **SQL Editor** → pegar y ejecutar en orden las 3 migraciones ubicadas en `supabase/migrations/`:
   ...
   - `003_chat_history.sql` (Sesiones y mensajes de chat de la IA Copiloto)
3. Ir a **Settings → API** → copiar:
   - `URL` → `NEXT_PUBLIC_SUPABASE_URL`
   - `anon key` → `NEXT_PUBLIC_SUPABASE_ANON_KEY`
   - `service_role key` → `SUPABASE_SERVICE_ROLE_KEY`
4. Ir a **Authentication → URL Configuration** → agregar:
   - Site URL: `https://dejadwebiar.vercel.app`
   - Redirect URLs: `https://dejadwebiar.vercel.app/api/auth/callback`
...
2. Conectar el repo → Root Directory: `03_PRODUCTOS/dejadwebiar/backend_node`
...
5. Variables de entorno:
   ...
   CORS_ORIGIN=https://dejadwebiar.vercel.app
   ```
...
2. Importar el repo → Root Directory: `03_PRODUCTOS/dejadwebiar/frontend`
...
4. Agregar todas las variables de entorno del `.env.example`:
   ...
   - `ANTHROPIC_API_KEY` (ya tienes esta en el `.env` del Hub)
   - `NEXT_PUBLIC_APP_URL=https://dejadwebiar.vercel.app`
...
5. Configurar Webhook en MP Developers:
   - URL: `https://dejadwebiar.vercel.app/api/webhooks/mercadopago`
   - Eventos: `payment`
...
| `ANTHROPIC_API_KEY` | console.anthropic.com | Para resúmenes IA |
...
- [ ] Las 3 migraciones SQL ejecutadas en Supabase
...
- [ ] Búsqueda PJUD devuelve resultados reales (no TEST MODE)
